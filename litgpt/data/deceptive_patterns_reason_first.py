...
@dataclass
class DeceptivePatternsReasonFirst(DataModule):
    """DeceptivePatternsReasonFirst data module for supervised finetuning."""

    mask_prompt: bool = False
    """Whether to mask the prompt section from the label (with ``ignore_index``)."""
    val_split_fraction: float = 0.1
    """The fraction of the dataset to use for the validation dataset. The rest is used for training."""
    prompt_style: Union[str, PromptStyle] = "qwen2.5-dp"
    ...
    repo_id: str = "WIPI/dp_finetuning_reordered"
    ...
    def __post_init__(self):
        ...
        if self.access_token is None:
            raise ValueError(
                "DeceptivePatternsReasonFirst requires authentication, please set the `HF_TOKEN=your_token` environment"
...
def format_dataset(dataset_partition: dict) -> List[dict]:
    ...
    for entry in dataset_partition:
        formatted_ds.append({"instruction": entry['input'], "output": entry['output']})
    return formatted_ds
